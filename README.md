...
Kết quả OCR luôn hiển thị ở màn hình **Duyệt trước khi đồng bộ**. Hệ thống chuẩn hóa biển số bằng cách bỏ dấu cách, dấu gạch và ký tự OCR thừa, sau đó đối chiếu ưu tiên theo số xe rồi theo tên tài xế. Chỉ những dòng khớp với danh sách hiện có mới được chọn mặc định. Nút **Chọn tất cả dữ liệu** cho phép chọn nhanh toàn bộ các dòng OCR để rà soát; dòng không khớp vẫn hiển thị nhưng không được ghi vào danh sách. Người dùng có thể bỏ chọn từng dòng rồi bấm **Đồng bộ N dòng** để cập nhật số liệu chuyến vào đúng `DriverRecord` và gửi toàn bộ danh sách lên `POST /api/fleet-data`.
