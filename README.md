...
Bấm **Ảnh chuyến** trên thanh công cụ để tải ảnh chụp bảng kê hoặc danh sách chuyến. Ảnh bảng dài được giữ độ phân giải và tự chia thành các lát dọc có phần chồng lấn, sau đó gửi tuần tự tới `/api/recognize-image`; Gemini Vision chạy hoàn toàn server-side và trả về tên tài xế, số xe, chuyến lớn, chuyến nhỏ, tổng chuyến, tổng km, xe nước và khối lượng trạm nếu có trong ảnh. Model mặc định là `gemini-3.6-flash`; có thể đổi bằng biến môi trường server `GEMINI_MODEL` khi Google phát hành model mới.

Kết quả OCR luôn hiển thị ở màn hình **Duyệt trước khi đồng bộ**. Hệ thống chuẩn hóa biển số bằng cách bỏ dấu cách, dấu gạch và ký tự OCR thừa, sau đó đối chiếu theo số xe. Không khử trùng lặp theo tên tài xế: một tài xế chạy hai xe sẽ được giữ thành hai dòng độc lập, mỗi dòng có bộ số liệu chuyến riêng. Mọi dòng OCR đều được giữ lại để rà soát; dòng thiếu tên hoặc số xe sẽ được đánh dấu và không tự động chọn đồng bộ. Các giá trị số dạng chuỗi, dấu phẩy/dấu chấm và `totalTrips` đều được chuẩn hóa ở client và kiểm tra lại ở server; khi có chuyến lớn hoặc chuyến nhỏ, tổng chuyến được tái tính bằng tổng hai cột này. Nút **Chọn tất cả dữ liệu** cho phép chọn nhanh các dòng hợp lệ; dòng có số xe chưa tồn tại sẽ được thêm mới khi đồng bộ. Người dùng có thể bỏ chọn từng dòng rồi bấm **Đồng bộ N dòng** để cập nhật số liệu chuyến vào đúng `DriverRecord` và gửi toàn bộ danh sách lên `POST /api/fleet-data`.

Ảnh gốc được giới hạn 20 MB, mỗi lát OCR không vượt quá 2,8 MB để phần base64 cùng metadata vẫn nằm dưới giới hạn request 4,5 MB của Vercel; máy chủ Express vẫn giới hạn payload 12 MB. Endpoint OCR yêu cầu đăng nhập, không nhận Gemini API key từ trình duyệt và không lưu ảnh gốc. Ảnh chỉ là dữ liệu đầu vào tạm thời; hệ thống chỉ lưu các số liệu chuyến đã được người dùng duyệt. Nút **Chụp bảng** trên thanh công cụ tải xuống ảnh PNG của bảng dữ liệu đang hiển thị, gồm tiêu đề, các dòng trên trang hiện tại và dòng tổng cộng của tập dữ liệu đang lọc.
