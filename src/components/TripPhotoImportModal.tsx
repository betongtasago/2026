...
import { CheckCircle2, FileImage, Loader2, ScanLine, Upload, X, Zap } from 'lucide-react';
...
function findMatchingRecord(row: OcrTripRow, records: DriverRecord[]): DriverRecord | undefined {
  const vehicle = normalizeStringForComparison(row.vehicleNumber);
  const driver = normalizeStringForComparison(row.driverName);
  return records.find((record) => {
    const sameVehicle = vehicle && normalizeStringForComparison(record.vehicleNumber) === vehicle;
      ...
      <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-2xl">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 overflow-y-auto lg:grid-cols-[300px_1fr]">
          <aside className="border-b border-slate-200 bg-slate-50/80 p-5 lg:border-b-0 lg:border-r">
            ...
            <p className="mt-3 text-center text-[11px] leading-5 text-slate-500">Ảnh được nén trước khi gửi. Không cần nhập API key trên trình duyệt.</p>
          ...
          <section className="flex min-h-[420px] flex-col p-5 sm:p-6">
            ...
            {!rows.length && !isReading && !error && <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 p-8 text-center"><FileImage className="h-10 w-10 text-cyan-600" /><p className="mt-4 text-sm font-black text-slate-800">Chưa có ảnh để nhận diện</p><p className="mt-1 max-w-sm text-xs leading-5 text-slate-500">Nên chụp rõ phần tiêu đề và các cột tên tài xế, số xe, chuyến lớn, chuyến nhỏ, tổng chuyến.</p></div>}
            ...
            {rows.length > 0 && !isReading && <div className="min-h-0 flex-1 overflow-auto rounded-2xl border border-slate-200"><table className="w-full min-w-[720px] text-left text-xs"><thead className="sticky top-0 bg-slate-950 text-[10px] font-black uppercase tracking-wider text-white"><tr><th className="w-12 px-3 py-3 text-center">Chọn</th><th className="px-3 py-3">Tài xế / số xe</th><th className="px-3 py-3 text-center">Lớn</th><th className="px-3 py-3 text-center">Nhỏ</th><th className="px-3 py-3 text-center">Tổng chuyến</th><th className="px-3 py-3 text-center">Tổng KM</th><th className="px-3 py-3">Đối chiếu</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map((row, index) => { const match = matches[index]; const selected = selectedIndexes.has(index); return <tr key={`${row.vehicleNumber}-${index}`} className={selected ? 'bg-cyan-50/60' : 'bg-white'}><td className="px-3 py-3 text-center"><input type="checkbox" checked={selected} disabled={!match} onChange={() => toggleIndex(index)} className="h-4 w-4 accent-cyan-700" /></td><td className="px-3 py-3"><p className="font-black text-slate-900">{row.driverName}</p><p className="mt-0.5 font-mono text-[11px] font-bold text-slate-500">{row.vehicleNumber}</p></td><td className="px-3 py-3 text-center font-black text-amber-600">{formatValue(row.largeTrips)}</td><td className="px-3 py-3 text-center font-black text-amber-600">{formatValue(row.smallTrips)}</td><td className="px-3 py-3 text-center font-black text-rose-600">{formatValue(row.totalTrips)}</td><td className="px-3 py-3 text-center font-black text-emerald-700">{formatValue(row.totalKm)}</td><td className="px-3 py-3">{match ? <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1 font-bold text-emerald-700"><CheckCircle2 className="h-3 w-3" />{match.vehicleNumber}</span> : <span className="font-bold text-rose-600">Không tìm thấy</span>}</td></tr>; })}</tbody></table></div>}
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4"><p className="flex items-center gap-1.5 text-xs text-slate-500"><Zap className="h-3.5 w-3.5 text-amber-500" />Chỉ các dòng có đối chiếu mới được đồng bộ.</p><div className="flex gap-2"><button type="button" onClick={handleClose} className="rounded-xl px-4 py-2.5 text-sm font-bold text-slate-600 transition hover:bg-slate-100">Hủy</button><button type="button" disabled={!selectedMatchedCount || isApplying} onClick={handleApply} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-black text-white shadow-lg shadow-emerald-600/20 transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50">{isApplying ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}Đồng bộ {selectedMatchedCount || ''} dòng</button></div></div>
          </section>
        </div>
      </div>
    </div>
